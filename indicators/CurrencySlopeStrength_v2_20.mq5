...
//+------------------------------------------------------------------+
//| Valor de forca por moeda na barra atual (k=0) p/ um TF.          |
int ComputeNow(ENUM_TIMEFRAMES tf, double &out[])
{
...
//+------------------------------------------------------------------+
void DrawCol(int win,int x,int y,int rh,int colW,ENUM_TIMEFRAMES tf,const double &V[],string cid)
{
   ...
   for(int r=0;r<8;r++)
   {
      ...
      Lbl(lblName,win,x,y+rh*(r+1),
          StringFormat("%-3s %+5.2f%s",cur[c],V[c],vis), shown);
      if(cid=="a")
      {
               ...
               const double &V4[],const double &V5[])
{
   ...
   Rect(PFX+"bg",win,x-6,y-6, panelW, rh*10+12, C'24,24,32');
   DrawCol(win,x,            y,rh,colW,InpTF1,V1,"a");
   DrawCol(win,x+colW,       y,rh,colW,InpTF2,V2,"b");
   DrawCol(win,x+colW*2,     y,rh,colW,InpTF3,V3,"c");
   DrawCol(win,x+colW*3,     y,rh,colW,InpTF4,V4,"d");
   DrawCol(win,x+colW*4,     y,rh,colW,InpTF5,V5,"e");
