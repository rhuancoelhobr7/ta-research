...
#property description "+ aba MATRIZ 8x8 por par (calculo do proprio CSS, nao o t do CSSM) + rotulos de ponta"
...
string PPFX= "CSS_p_";  // objetos do painel-normal
string MPFX= "CSS_mx_"; // objetos da aba MATRIZ
...
int ComputePairVals(ENUM_TIMEFRAMES tf, int kShift, double &pv[], bool &pok[])
{
   ...
   for(int p=0;p<gPairsN;p++)
   {
      ...
      double price=cl[kShift]; if(price==0) continue;
      double z=((tma[kShift]-prev)/price)/(norm+1e-12);
      pv[p]=Clamp(z*InpScale,-(InpScaleMax-0.02),(InpScaleMax-0.02));
      pok[p]=true; good++;
   }
   return good;
}
               ...
               const double &V4[],const double &V5[])
{
   ...
   Lbl(PPFX+"hd",win,x,yTop,
       StringFormat("CSS FORCA (TMA)  linhas:%s  box:%.2f",
                    TfStr(InpLineTF),InpBox), clrWhiteSmoke);
...
void DrawEndLabels(int win)
{
   ...
   datetime tEnd=iTime(_Symbol,_Period,0)+PeriodSeconds(_Period);
